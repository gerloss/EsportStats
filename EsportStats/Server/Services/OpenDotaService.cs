...
namespace EsportStats.Server.Services
{
    public interface IOpenDotaService
    {
        ...
        public Task<IEnumerable<TopListEntryExtDTO>> GetTopListEntriesAsync(IEnumerable<IDotaPlayer> players, Metric metric);
    }
    ...
    {        
        ...
        /// <summary>
        /// Gets the hero statistics of a list of players
        /// </summary>   
        ...
        /// <summary>
        /// Gets the top list entries for a metric for a list of players
        /// </summary>   
        public async Task<IEnumerable<TopListEntryExtDTO>> GetTopListEntriesAsync(IEnumerable<IDotaPlayer> players, Metric metric)
        {
            var topListEntriesBaseUrl = "https://api.opendota.com/api/players/";
            var httpClient = _httpClientFactory.CreateClient();
            SetMaxConcurrency(topListEntriesBaseUrl, _openDotaOptions.BatchSize);

            var urls = players.Select(player => topListEntriesBaseUrl
                + player.SteamId.ToSteam32()
                + "/matches?sort=" + metric.GetShortName()
                + "&limit=10"
                + "&api_key=" + _openDotaOptions.Key
            );
            ...
            for (int i = 0; i < numberOfBatches; i++)
            {
                var batchOfUrls = urls.Skip(i * _openDotaOptions.BatchSize).Take(_openDotaOptions.BatchSize);
                var batchOfRequests = batchOfUrls.Select(url => httpClient.GetAsync(url));
                responses.AddRange(await Task.WhenAll(batchOfRequests));
            }

            var results = new List<TopListEntryExtDTO>();
            ...
            {                                                                                                   //  [0]   [1]      [2]         [3]             [4]
                ...
                {
                    var responseString = await response.Content.ReadAsStringAsync();
                    var parsedResponse = JsonConvert.DeserializeObject<List<TopListEntryExtDTO>>(responseString);
                    foreach(var entry in parsedResponse)
                    {
                        entry.User = players.SingleOrDefault(p => p.SteamId == steamId32.ToSteam64());
                    }
                    results.AddRange(parsedResponse);
                ...
                {
                    throw new HttpRequestException("Unsuccessful request towards the OpenDota API: " + topListEntriesBaseUrl);
                }
            }
